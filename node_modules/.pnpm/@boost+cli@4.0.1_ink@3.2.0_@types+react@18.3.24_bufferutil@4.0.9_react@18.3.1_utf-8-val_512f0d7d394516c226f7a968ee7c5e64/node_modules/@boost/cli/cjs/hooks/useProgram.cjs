...
const ProgramContext = require('../ProgramContext.cjs');
function useProgram() {
  ...
  const exit = React.useCallback((error, code) => {
    if (error) {
      appExit(new common.ExitError(error instanceof Error ? error.message : error, code ?? constants.EXIT_FAIL));
    } else {
...
//# sourceMappingURL=useProgram.cjs.map
